...
// need consistent multi-entity views (overview aggregation, bootstrap).
func (s *Store) Snapshot() State {
	s.mu.RLock()
	defer s.mu.RUnlock()
	out := State{Version: s.state.Version}
	out.Zones = make([]domain.FarmZone, len(s.state.Zones))
	for i, z := range s.state.Zones {
		out.Zones[i] = cloneFarmZone(z)
	}
	out.Buoys = make([]domain.Buoy, len(s.state.Buoys))
	for i, b := range s.state.Buoys {
		out.Buoys[i] = cloneBuoy(b)
	}
	out.Samples = make([]domain.WaterSample, len(s.state.Samples))
	for i, sm := range s.state.Samples {
		out.Samples[i] = cloneWaterSample(sm)
	}
	out.Warnings = make([]domain.WarningRecord, len(s.state.Warnings))
	for i, w := range s.state.Warnings {
		out.Warnings[i] = cloneWarningRecord(w)
	}
	out.Aeration = make([]domain.AerationLog, len(s.state.Aeration))
	for i, a := range s.state.Aeration {
		out.Aeration[i] = cloneAerationLog(a)
	}
	out.FarmLogs = make([]domain.FarmLog, len(s.state.FarmLogs))
	for i, f := range s.state.FarmLogs {
		out.FarmLogs[i] = cloneFarmLog(f)
	}
	out.Audit = make([]domain.AuditEntry, len(s.state.Audit))
	for i, a := range s.state.Audit {
		out.Audit[i] = cloneAuditEntry(a)
	}
